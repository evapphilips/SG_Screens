// Eva Philips
// June 19, 2019
// This code documents the possibility to determine location and distance from the posenet model. 
// References:
// 1. This code is heavly based off of this reference code from the ml5.js website - https://ml5js.org/reference/api-PoseNet/
// 2. Posenet keypoint reference - https://github.com/tensorflow/tfjs-models/tree/master/posenet
    ...
    for(let i=0; i<poses.length; i++){
        ...
        for(let j=0; j<pose.keypoints.length; j++){
            ...
            if(keypoint.score>0.2){
                ...
                // Change color when hands are far apart from each other
                let leftHand = createVector(pose.keypoints[9].position.x, pose.keypoints[9].position.x);
                let rightHand = createVector(pose.keypoints[10].position.x, pose.keypoints[10].position.x);
                let d = rightHand.dist(leftHand);
                if(d>500){
                    fill(255, 0, 0);
                }else{
                    fill(255);
